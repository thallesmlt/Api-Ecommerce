...
class ClienteController {
    ...
    async index(req, res, next) {
        ...
        try {
            const clientes = await Cliente.paginate({ loja: req.query.loja }, { offset, limit, populate: "usuario" }); //ref é pra o populate identificar o usuario
    ...
    searchPedidos(req, res, next) {
        return res.status(400).send({ error: "Desenvolvimento pendente" });
    }
    ...
    async search(req, res, next) {
        ...
        try {
            const clientes = await Cliente.paginate({ loja: req.query.loja, nome: { $regex: search } }, { offset, limit, populate: "usuario" }); //ref é pra o populate identificar o usuario
            return res.send({ clientes });
        } catch (e) {
            next(e);
        }
    ...
    async showAdmin(req, res, next) {
        try {
            const cliente = await Cliente.findOne({ _id: req.params._id, loja: req.query.loja }).populate({ path:"usuario", select: "-salt -hash" });
    ...
    */
     ...
     async show (req,res,next){
        try {
            const cliente = await Cliente.findOne({ usuario: req.payload.id, loja: req.query.loja }).populate({ path:"usuario", select: "-salt -hash" }); //nao deve retornar salt e hash
            return res.send({ cliente });
        }catch(e){
            next(e);
        }
